...
from classifier import StateBehaviorClassifier # Import the classifier for variance calculation
...
class RectangularTrueBlochMatcher:
    ...
    def run(self, output_path: Optional[str] = None):
        ...
        # --- VARIANCE-AWARE CONDITIONAL ALIGNMENT LOGIC ---
        ...
        variance = classification_info['variance']
        
        alignment_variance_threshold = self.analysis_kwargs.get("alignment_variance_threshold", 1.0)
        delta = 0.0
        
        if variance >= alignment_variance_threshold:
            print(f"[ALIGN] High variance ({variance:.3f} eV^2) detected for low-energy metal states, indicating strong bonding.")
            print("[ALIGN] Aborting metal-based alignment. Aligning full system to its own Fermi energy.")
        else:
            print(f"[ALIGN] Low variance ({variance:.3f} eV^2) detected. Proceeding with metal-based alignment.")
...
if __name__ == "__main__":
    run_kwargs = {
        ...
        "reuse_cached": True,
        "alignment_variance_threshold": 1.0 # in eV^2
    }
    match_files = run_match(
        molecule_dirs=[r'dir',r'dir'],
        metal_dir=r'dir',
        full_dir=r'dir',
        **run_kwargs
    )
    ...
    print("\n--- Running Plotter ---")
    try:
        # === UPDATED: You can now choose the coloring mode here ===
        # pick_primary = True      # --> Winner-Take-All (solid colors)
        # pick_primary = False     # --> N-Segment view with threshold (default)
        # pick_primary = "blended" # --> Accurate weighted mix of all overlaps
        ...
        cfg = PlotConfig(
            ...
            energy_range=(-3, 3.125), 
            shared_molecule_color=True,
            min_total_mol_wspan=0.025,
            pick_primary=False # <--- SET YOUR DESIRED MODE HERE
        )
